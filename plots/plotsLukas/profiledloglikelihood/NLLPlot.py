...
for var1 in range( binningX[1], binningX[2], ( binningX[2] - binningX[1]) / binningX[0] ):
    for var2 in range( binningY[1], binningY[2], ( binningY[2] - binningY[1]) / binningY[0] ):
        ...
        signal_rate                  = {}
        for i_region, region in enumerate(regions):
            signal_rate[region] = w.get_weight_yield( ttX_coeffList[region], **kwargs) 

            bin_name = "Region_%i" % i_region
            nice_name = region.__str__()
            c.addBin(bin_name,[s.name for s in bg], nice_name)
...
plotting.draw2D( nllPlot, plot_directory = plot_directory_, extensions = ["png", "root", "pdf"])
