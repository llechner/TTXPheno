...
#nohup krenew -t -K 10 -- bash -c "python ${script}.py ${small} --process ttgamma_1l --selection ${selection} --sample fwlite_ttgammaLarge_LO_order2_15weights_ref --level ${level} --version ${version} --variables ctZ ctZI --zRange ${zRange} --luminosity 150 --binning 31 -0.5 0.5 31 -0.5 0.5 --smooth --contours --cores 2 --fit BestFit ${overwrite} --binMultiplier ${multiplier}" &
...
#nohup krenew -t -K 10 -- bash -c "python ${script}.py ${small} --process ttgamma_2l --selection ${selection} --sample fwlite_ttgammaLarge_LO_order2_15weights_ref --level ${level} --version ${version} --variables ctZ ctZI --zRange ${zRange} --luminosity 150 --binning 31 -0.5 0.5 31 -0.5 0.5 --smooth --contours --cores 3 --fit BestFit ${overwrite} --binMultiplier ${multiplier}" &

#CuB CuW
#nohup krenew -t -K 10 -- bash -c "python ${script}.py ${small} --process ttgamma_2l --selection ${selection} --sample fwlite_ttgammaLarge_LO_order2_15weights_ref --level ${level} --version ${version} --variables cuB cuW --zRange ${zRange} --luminosity 150 --binning 31 -2 2 31 -2 2 --smooth --contours --cores 3 --fit BestFit ${overwrite} --binMultiplier ${multiplier}" &

#combined
##nohup krenew -t -K 10 -- bash -c "python ${script}_combined.py ${small} --level ${level} --version ${version} --variables ctZ ctZI --binning 31 -0.3 0.3 31 -0.3 0.3 --luminosity 150 --fit BestFit --zRange ${zRange} --contours --smooth --cores 6 ${overwrite} --binMultiplier ${multiplier}" &
