...
colorsNonInfo = [ ROOT.kRed, ROOT.kRed, ROOT.kRed, ROOT.kBlack]
...
if args.processFile.split('_')[0] == 'ttgamma':
    ...
    ttgammaTopSample  = copy.deepcopy( ttSample ) #select ttgamma events with isolated gamma from ISR (cat a2)
    ttgammaTopSample.name = 'fwlite_ttgamma_top_LO_order2_15weights_ref'
...
if 'ttZ' in args.processFile.split('_'):
    ...
    # be careful if you set nonInfo to empty list, especially with the FI plot
    if args.noninfoSignal or args.addFisherInformationBackground: nonInfo = [ ttgammaIsrSample, ttgammaLepSample, ttgammabSample, ttgammaTopSample ]
...
if args.processFile.split('_')[0] == 'ttgamma':
    ...
    ttXSample.addSelectionString(         "signalPhoton==1&&abs(genPhoton_motherPdgId[0])!=6" ) #cat a1 <- the one and only signal
    ...
    if args.noninfoSignal or args.addFisherInformationBackground:
        ttgammaTopSample.read_variables  = catPhoton_variables
        ...
        ttgammaTopSample.addSelectionString(  "signalPhoton==1&&abs(genPhoton_motherPdgId[0])==6"    ) #cat a2
