...
argParser.add_argument('--version',         action='store',     default='xsecTest', help='Appendix to plot directory') 
...
#argParser.add_argument('--selection',       action='store',     default='lepSel3-onZ-Zpt0', help="Specify cut.")
argParser.add_argument('--selection',       action='store',     default='lepSel1-gammapt40', help="Specify cut.")
...
params.append( [{'legendText':'ref', 'WC':{}, 'color':ROOT.kBlack}] )
params.append( [{'legendText':'no ref', 'WC':{}, 'color':ROOT.kRed}] )
...
ttXSampleNoRef = getattr( loadedSamples, args.sample.replace('_ref','') )

if args.small:
    ttXSample.reduceFiles( to = 5 )
    ttXSampleNoRef.reduceFiles( to = 5 )
...
ttXSample.event_factor = ttXSample.nEvents / float( ttXSample.chain.GetEntries() )
ttXSample.setSelectionString( cutInterpreter.cutString(args.selection) )
ttXSample.read_variables = ["ref_lumiweight1fb/F"]
ttXSample.read_variables.append( VectorTreeVariable.fromString('p[C/F]', nMax=2000) )

ttXSampleNoRef.event_factor = ttXSampleNoRef.nEvents / float( ttXSampleNoRef.chain.GetEntries() )
...
if args.reweightPtXToSM:
    ...
    def get_reweight( param , sample_ ):
        ...
        print sample_.name
        print checkReferencePoint( sample_ )
        print sample_.xsec * 1000. / sample_.nEvents * sample_.event_factor
        print 
