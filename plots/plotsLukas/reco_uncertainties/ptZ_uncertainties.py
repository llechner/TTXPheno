...
from TTXPheno.Analysis.regions import recottZRegionsPTZ as regions
...
argParser.add_argument('--cardFile',           action='store',     default='PTZ_SM_cardfile2.txt', help='Cardfile where additional uncertainties are taken from')
...
maxval = 400
for s in [ttXSample] + bg + nonInfo:
    hists[s.shortname] = ROOT.TH1F(s.shortname,"", Nbins, 0, maxval)
if args.nonInfoSignal:
    hists['nonInfo'] = ROOT.TH1F('nonInfo',"", Nbins, 0, maxval)
if args.parameters is not None:
    for param in params:
        hists['signal_%s'%param['name']] = ROOT.TH1F('signal_' + param['name'],"", Nbins, 0, maxval)
...
#        ROOT.gStyle.SetHatchesLineWidth(3)
    ...
    print 'region', i_region, 'tot unc', totalUncertainty, 'tot rate', rate[region][ttXSample.shortname] + rate[region]['nonInfo'] + sum([rate[region][s.shortname] for s in bg])
...
plot = Plot.fromHisto(plotName, plots, texX = "p_{T}(ll) (GeV)", texY = "Number of Events / 40 GeV" )
...
for ib in range(1, 1 + hists[ttXSample.shortname].GetNbinsX() ):
    val = hists[ttXSample.shortname].GetBinContent(ib) + hists['nonInfo'].GetBinContent(ib) if args.nonInfoSignal else hists[ttXSample.shortname].GetBinContent(ib)
    if val<0: continue
