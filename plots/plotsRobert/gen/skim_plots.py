...
def NanJet():
    return {'pt':float('nan'), 'phi':float('nan'), 'pdgId':float('nan'), 'eta':float('nan'), 'motherPdgId':float('nan'), 'matchBParton':float('nan')}
...
def makeJets( event, sample ):
    ...
    event.bj0, event.bj1 = (event.trueBjets + event.trueNonBjets)[:2]
